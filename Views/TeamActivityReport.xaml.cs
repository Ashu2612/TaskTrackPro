...
public partial class TeamActivityReport : ContentView
{
    public TeamActivityReport()
    {
        InitializeComponent();
        ProjectCollection();
    ...
    private WorkItemMetrics GetWorkItemMetrics(string name)
    {
        try
        {
            ...
            return CalculateMetrics(connection, name);
        }
        catch (Exception ex) { }
       return null;
    }

    private WorkItemMetrics CalculateMetrics(VssConnection connection, string Name)
    {
        try
        {
            ...
            var wiql = new Wiql()
            {
                Query = $"SELECT [System.Id], [System.Title], [System.State], [System.WorkItemType], [Microsoft.VSTS.Scheduling.CompletedWork] FROM WorkItems WHERE [System.TeamProject] = '{CommonClass.projectDetails.SelectedProject.ProjectName}' AND [System.AssignedTo] = '{Name}'"
            };
    ...
    private void SearChTB_Unfocused(object sender, FocusEventArgs e)
    {
        var metrics = GetWorkItemMetrics(SearChTB.Text);
        CreateChart(metrics);
    }
}